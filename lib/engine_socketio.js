...
function SocketIoEngine(script) {
  ...
  if (script.config.socketio) {
    ...
    if (script.config.socketio.auth) {
      this.auth = script.config.socketio.auth;
    }
...
SocketIoEngine.prototype.step = function (requestSpec, ee) {
  ...
  let f = function(context, callback) {
    ...
    if (isResponseRequired(requestSpec)) {
      ...
      const data = Array.isArray(outgoing.data) ? outgoing.data : [outgoing.data];
      debug('sending:', [outgoing.channel, ...data]);
      socketio.emit(outgoing.channel, ...data, function(err, result) {
        if (!err) {
...
SocketIoEngine.prototype.loadContextSocket = function(namespace, context, cb) {
  ...
  if(!context.sockets[namespace]) {
    ...
    context.sockets[namespace].on('connect', function(){
      if (auth) {
        let channel = Object.keys(auth)[0];
        context.sockets[namespace].emit(channel, auth[channel], function(){
          debug('authenticated');
